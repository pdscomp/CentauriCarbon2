...
void gpio_peripheral(uint32_t gpio, uint32_t mode, int pullup)
{
    ...
    else if (func == 2)
    {
        ...
        else if (gpio == GPIO('C', 6) || gpio == GPIO('C', 7) || gpio == GPIO('C', 8) || gpio == GPIO('C', 9))
            stm32f1_alternative_remap(AFIO_MAPR_TIM3_REMAP_Msk,
                                      AFIO_MAPR_TIM3_REMAP_FULLREMAP);
        else if (gpio == GPIO('D', 12) || gpio == GPIO('D', 13) || gpio == GPIO('D', 14) || gpio == GPIO('D', 15))
    ...
    {
        // SPI
        if (gpio == GPIO('B', 3) || gpio == GPIO('B', 4) || gpio == GPIO('B', 5))
