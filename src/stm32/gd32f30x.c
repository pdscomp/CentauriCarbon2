...
// 默认时钟配置下：AHB 120MHZ APB2 60MHZ APB1 60MHZ
// APB2总线外设 USART0,SPI0,ADC0~2,EXTI,GPIOA~G,TIMER0,TIMER7,TIEMR8~10
// APB1总线外设 CAN0,WWDGT,TIMER1~3,SPI1~2,I2S1~2,I2C0,I2C1,USBD,FWDGT,RTC,DAC,TIMER4~6,UART3~4,TIMER11~13,CTC
...
clock_setup(void)
{
    // 时钟在SystemInit函数配置，/* CK_PLL = (CK_HXTAL/2) * 30 = 120 MHz */
#if 0
...
void bootloader_request(void)
{
    ...
    // 设置备份区标记请求
    irq_disable();
    bootloader_set_request(0, 0x454C);
    NVIC_SystemReset();
}

void bootloader_set_request(uint8_t sec, uint16_t flag)
{
    // 写入备份区标记请求
    rcu_periph_clock_enable(RCU_PMU);
    rcu_periph_clock_enable(RCU_BKPI);
    pmu_backup_write_enable();
    BKP_DATA0_9(sec) = flag;
    pmu_backup_write_disable();
}

uint16_t
bootloader_get_request(uint8_t sec)
{
    // 读取备份区标记请求
    rcu_periph_clock_enable(RCU_PMU);
    rcu_periph_clock_enable(RCU_BKPI);
    return BKP_DATA0_9(sec);
}
