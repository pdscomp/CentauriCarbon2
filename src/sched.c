...
#include "version.h"
static const char mcu_version[] __attribute((used)) = "MCU_VERSION_MAGIC=" MCU_VERSION;
...
void
__sched_add_timer(struct timer *add, const char *func)
{
    ...
    if (unlikely(timer_is_before(waketime, tl->waketime))) {
        ...
        if (timer_is_before(waketime, time))
        {
            DEBUG("Timer too close %u %u\n", waketime, time);
            output("Timer too close %u %u %s", waketime, time, func);
