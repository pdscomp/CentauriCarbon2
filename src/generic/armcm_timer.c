...
#if CONFIG_DEBUG_LED >= 0

static struct timer debug_led_timer;
static struct gpio_out debug_led_pin;

static uint_fast8_t debug_led_event(struct timer *t)
{
    gpio_out_toggle(debug_led_pin);
    debug_led_timer.waketime += timer_from_us(500000 / CONFIG_DEBUG_LED_FREQ);
    return SF_RESCHEDULE;
}

void debug_led_init(void)
{
    debug_led_pin = gpio_out_setup(CONFIG_DEBUG_LED, 1);
    sched_del_timer(&debug_led_timer);
    debug_led_timer.waketime = timer_read_time() + timer_from_us(500000 / CONFIG_DEBUG_LED_FREQ);
    debug_led_timer.func = debug_led_event;
    sched_add_timer(&debug_led_timer);
}
...
timer_init(void)
{
    ...
    irq_restore(flag);
        ...
        debug_led_init();
    #endif
    // 编译脚本会将该初始化放到timer_init之前导致时序出错,所以放在这里确保不会有问题
...
timer_dispatch_many(void)
{
    ...
    for (;;) {
        ...
        if (unlikely(timer_is_before(tru, now))) {
            ...
            {    
                DEBUG( "Rescheduled timer in the past %d\n", diff);
