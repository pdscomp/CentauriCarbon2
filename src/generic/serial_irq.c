...
#else
    bootloader_commit_data((uint8_t *)&data, 1);
#endif
}
...
void console_task(void)
{
    ...
    {
#if 0
        DEBUG( "msg recv %d:   ", pop_count);
        for(int i = 0; i < pop_count; i++)
            DEBUG( "%x ", receive_buf[i]);
        DEBUG( "\n");
#endif
        if (CONFIG_HAVE_BOOTLOADER_REQUEST && ret < 0 && pop_count == 32 && !memcmp(receive_buf, " \x1c Request Serial Bootloader!! ~", 32))
            bootloader_request();
...
#else
// Encode and transmit a "response" message
void bootloader_send_data(uint8_t *msg, uint16_t len)
{
    // Verify space for message
    uint_fast8_t tpos = readb(&transmit_pos), tmax = readb(&transmit_max);
    if (tpos >= tmax)
