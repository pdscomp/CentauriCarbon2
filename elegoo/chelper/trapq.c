...
void __visible
trapq_debug(struct trapq *tq)
{
    struct move *m;
    list_for_each_entry(m, &tq->moves, node) {
        errorf("debug  print_time=%f, move_t=%f, start_v=%f, half_accel=%f, start_x=%f, start_y=%f, start_z=%f, axes_r_x=%f, axes_r_y=%f, axes_r_z=%f",
           m->print_time, m->move_t, m->start_v,m->half_accel,m->start_pos.x,m->start_pos.y,m->start_pos.z,m->axes_r.x,m->axes_r.y,m->axes_r.z);
    }
}
